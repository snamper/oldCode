...
//		    if (Password.indexOf("(@*@)") == 0 ) ConnectionUrl = Encrypts.uncrypt(Password.substring(5), "password");
		...
		try{
			...
			for (int i=0; i < list.size(); i++){
				...
				if (con.DriverClass.equals(DriverClass) && con.ConnectionUrl.equals(ConnectionUrl)
						...
						&& con.dc == dc){
					con.UseCount ++;
					fc.message("[ConnectPool]现有连接:" + con + "[" + (i + 1) + "/" + list.size() + "]");
					PrintConnect();
			...
			con.UseCount ++;
			list.add(con);
			fc.message("[ConnectPool]新建连接:" + con + "[" + (list.indexOf(con) + 1) + "/" + list.size() + "]");
			//
			PrintConnect();
			return newConn;
		}catch(Exception e){
	...
	public synchronized static void CheckOutTimeConnect(boolean isAll){
		...
		while (i > 0){
			...
			Date date = new Date(System.currentTimeMillis() - 1 * 60 * 1000);	//半小时之前 
