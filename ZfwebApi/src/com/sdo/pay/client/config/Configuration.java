...
package com.sdo.pay.client.config;

import java.io.IOException;
import java.io.InputStream;
import java.util.Properties;

import com.sdo.pay.client.config.enums.Bank;
...
//    public static String bankCode;
    ...
    static {
		...
		Configuration.merchantNo = getValue(p , "merchantNo"
				,"用户必须配置自己的商户号码（修改config.properties文件的__key键.）<br/>");
		
		Configuration.key = getValue(p , "MD5Key" 
				, "用户必须配置自己的MD5 key值（修改config.properties文件的__key键.）<br/>");
		
		Configuration.paymentGateWayURL = getValue(p , "paymentGateWayURL" 
				, "用户应该根据开发环境（测试或者正式）配置相应的支付网关地址（修改config.properties文件的__key键.）<br/>");
...
//						"（修改config.properties文件的__key键.）<br/>");
		...
		Configuration.payChannel = getValue(p , "payChannel" 
				, "用户应该根据自己开通的相应渠道配置好渠道（修改config.properties文件的__key键.）<br/>");
		...
		Configuration.notifyURL = getValue(p , "notifyURL" 
				, "用户必须配置发货通知地址，否则盛付通系统无法通知您发货（修改config.properties文件的__key键.）<br/>");
		
		Configuration.postBackURL = getValue(p , "postBackURL" 
				, "用户必须配置显示回调地址，否则无法显示支付成功信息给客户（修改config.properties文件的__key键.）<br/>");
