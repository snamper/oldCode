...
public class DataConnect {
	...
	public void loadDbConfig(String sJndiName){ 
		try {    
		    ...
		    if (!b) System.out.print("无效的jndi名称:" + sJndiName); 
		} catch (Exception e) {    
		   Error.oute(e);    
		}
	}
	...
	private ResultSet queryBuffer(Statement stmt, String sql, String KeyFieldName, int nBeginRow, int nReadCount){
		try{
			//用主键查询
			String sOldSelect = funcejb.getString(sql.toLowerCase(), "select", "from");
			String sNewSql = "SELECT " + KeyFieldName + " FROM " + funcejb.getString(sql.toLowerCase() + "_|_|_!@#$%^&*()_|_|_", "from", "_|_|_!@#$%^&*()_|_|_");
			ResultSet rs = stmt.executeQuery(sNewSql);
			if (rs == null) return null;
			int n = 0;
			String sIdList = "";
			while (rs.absolute(nBeginRow) && (n < nReadCount)){
				sIdList = sIdList + ",'" + funcejb.gets(rs, KeyFieldName, "") + "'";
				//
				n++;
				rs.next();
			}
