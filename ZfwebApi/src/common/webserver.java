...
public class webserver {
	//static DataConnect conn = new DataConnect();
	/*
	 * 检测验证码
	 */
	public static boolean checkCode(HttpServletRequest request, String checkCode)
		...
		try {
			...
			System.out.print("session=" + session.toString());
			if (session.getAttribute("checkcode") != null){
				s = session.getAttribute("checkcode").toString();
				System.out.print("比较验证码:s=" + s + ", checkCode=" + checkCode);
				return (s.equals(checkCode));
			}else
				return false;
		} catch (Exception e) {
			//msg.log("webserver.checkCode", 3, e.toString());
			return false;
		}
	}
	...
	public static void logonPopedom(HttpServletRequest request,
		...
		try {
			...
			System.out.print("[logon popedom]" + session.getId() + "," + name);
		} catch (Exception e) {
			System.out.print(e.toString());
		}
	}
	...
	public static void logoutPopedom(HttpServletRequest request,
		...
		try {
			...
			System.out.print("[logout popedom]" + session.getId() + "," + name);
		} catch (Exception e) {
			System.out.print(e.toString());
		}
	}
...
//	public static int checkPopedomMember(HttpServletRequest request, String sFunc, String sActive){
//		try {
//			String s = getLogonName(request);
//			if (s.equals("admin")) return 0;
//			DataAccess da = new DataAccess();
//			String sql = "SELECT * FROM userpopedom INNER JOIN Member ON userpopedom.fUserId = Member.id INNER JOIN" +
//				" popedom ON userpopedom.fPopedomId = popedom.id WHERE (popedom.fFunc = '" + sFunc +
//				"') AND (popedom.fActive = '" + sActive + "') and CONVERT(varchar(255), fName)=\'" + s + "\'";
//			ResultSet rs = da.queryRs(sql);
//			if (rs.next())
//				return 0;
//			else
//				return -1;
...
//	}
	...
	public static void getCheckCode(HttpServletRequest request,
			...
			java.io.IOException {
		System.out.print("[PayAdmin.webserver.getCheckCode]session=" + request.getSession().toString());
