	...
	public int query(String sql, boolean useCheckQuery, String keyField, boolean IsFast) {
		try {
			...
			if (useCheckQuery){				//在这里只用id查询
				...
				System.out.print("query:" + sNewSql);
				conn.CloseResultSet(rs);
...
//			System.out.println("超出结果集范围");
	 ...
	 * 返回List数据
	 */
	public List<String[]> getList(ResultSet rs, int nPage){
		try{
			//填入List
			List<String[]> list = new ArrayList<String[]>();
			ResultSetMetaData rsmd = rs.getMetaData();
			int nFieldCount = rsmd.getColumnCount();
			//保存列名
			String[] objRow = new String[nFieldCount];
			for(int nCol=0; nCol < nFieldCount; nCol++){
				objRow[nCol] = rsmd.getColumnName(nCol + 1).toLowerCase();
			}
			list.add(objRow);
			//保存字段内容
			for(int nRow=1; nRow <= nPageRowCount; nRow++){
				if (toRow(nPage, nRow)){
					objRow = new String[nFieldCount];
					for(int nCol=0; nCol < nFieldCount; nCol++){
						objRow[nCol] = rs.getString(nCol + 1);
					}
					list.add(objRow);
				}else
					break;
