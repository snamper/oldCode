...
//		return sPrefix + fc.getTime("yyMMddHHmmss") + r.substring(1);
	...
	public static String SendDataViaGet2(String sUrl, String sCharset) {
		 try{
             ...
             for(int i=0;i<count;i++)
                result[i]=buff[i];                     
             if (sCharset.equals("")) sCharset = "GBK";
             String output=new String(result, sCharset);
           	 return output;
	     }catch (Exception e){
	    	 return "";
	     }	
	...
	public static String getString2(String sData, String sBeginStr, String sEndStr){
		int n1 = sData.toLowerCase().indexOf(sBeginStr.toLowerCase());
		if (n1 > -1){
			sData = sData.substring(n1 + sBeginStr.length());
			//
			int n2 = sData.toLowerCase().indexOf(sEndStr.toLowerCase());
			if (n2 > -1){
				return sData.substring(0, n2);
			}else
				return "";
		}else
			return "";
	...
	public static String delString(String s, String sBegin, String sEnd) {
		int n1 = s.indexOf(sBegin);
		int n2 = s.indexOf(sEnd);
		while ((n1 > -1) && (n2 > -1) && (n2 > n1)){
			s = s.substring(0, n1) + s.substring(n2 + sEnd.length());
			n1 = s.indexOf(sBegin);
			n2 = s.indexOf(sEnd);
		}
