 ...
 */
 public class PayQuery extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	...
	public String payQuery(HttpServletRequest request, HttpServletResponse response){
		try{
			//1.操作参数
			String type = fc.getpv(request, "type");	//0取卡, 1回写
			...
			if (type.equals("0")){
				...
				//锁定
//				String sql = "update CaRenotice set fLockID = '" + appid + "' where " +   
//				" fid in (select top " + count + " fid from CaRenotice where fOverTime <= getdate() and fLockID in ('', '" + appid + "'))";
//				int n = dc.execute(sql);
//				System.out.print("[" + appid + "]锁定:" + n);	// fLockID = '" + appid + "' and
				//取回
				String sList = "";
				String sql = "select top " + count + " * from CaReNotice where  fOverTime <= getdate() " + order;
			...
			if (type.equals("1")){
				...
				if (state.equals("0")){	
					//如果成功,更新主订单状态 
					String sql = "update CaOrder set fNoticeState = '2' where fid = '" + fid + "'";
					int n = dc.execute(sql);
					if (n == 1){
						//从重发列表中清除
						sql = "delete from CaReNotice where fid = '" + fid + "'";
						n = dc.execute(sql);
						System.out.print(n);
					}
				}else{
					//如果失败,延时等待下次进行重发
					String sql = "update CaReNotice set flockid = '', fovertime = getdate() + 0.0000116 * fReInterval, fResult = '" + error + "' where fid = '" + fid + "'";
					dc.execute(sql);
				}
			}
