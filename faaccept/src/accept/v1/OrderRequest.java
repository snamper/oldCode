...
//		out.print(new accept.v2.OrderRequest().orderRequest(request, response));
		out.print("调用错误");
	}   	
	...
	public String orderRequest(HttpServletRequest request, HttpServletResponse response){
		try{
			System.out.println("----暂停使用，如果看到这句话，说明调用错误-----");
...
//			if (fIsSaveMainTable.equals("")){
				...
				String ss = oc.orderCheck2(fSource, fc.getGUID6(""), "", fClientID, fClientOrderID, fProductID, fPrice, 
						...
						fClientIp, fClientArea, fClientTime, fInfo1, fInfo2, fInfo3, fClientReUrl, AgentID, "");
...
//			sis = fc.SetSqlInsertStr(sis, "fOrderID", fClientOrderID);
...
//			sis = fc.SetSqlInsertStr(sis, "fUserIP", fClientIp);
//			sis = fc.SetSqlInsertStr(sis, "fUserArea",fClientArea);
...
//			sis = fc.SetSqlInsertStr(sis, "fNoticeUrl", fClientReUrl);
...
//			if (m == -2){
//				////dc.CloseConnect();
//				return fc.getResultStr("response", "false", "订单已存在,充值中");
//			}
//					
//			//5.完成
//			////dc.CloseConnect();
